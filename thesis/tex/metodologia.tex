...
\begin{equation}
   \nabla^2 u(\mathbf{x}_b) \approx \sum_{j=1}^N \omega_j^{\nabla^2} u_j + \omega_g^{\nabla^2} u_g
   \label{eq:laplacian_with_ghost}
\end{equation}

\noindent onde $u_j = u(\mathbf{x}_j)$ são os valores da função nos nós do estêncil, $u_g = u(\mathbf{x}_g)$ é o valor no nó fantasma, e $\omega_j^{\nabla^2}$ e $\omega_g^{\nabla^2}$ são os pesos RBF-FD calculados normalmente com o estêncil estendido. Da mesma forma, obtemos pesos para as derivadas parciais: $\omega_j^x$, $\omega_g^x$, $\omega_j^y$ e $\omega_g^y$.
...
\begin{align}
   \nabla^2 u(\mathbf{x}_b) &\approx \sum_{j=1}^N \omega_j^{\nabla^2} u_j + \omega_g^{\nabla^2} u_g \nonumber \\
   &= \sum_{j=1}^N \omega_j^{\nabla^2} u_j + \omega_g^{\nabla^2} \left( -\frac{1}{\omega_g^n} \sum_{j=1}^N \omega_j^n u_j \right) \nonumber \\
   &= \sum_{j=1}^N \omega_j^{\nabla^2} u_j - \frac{\omega_g^{\nabla^2}}{\omega_g^n} \sum_{j=1}^N \omega_j^n u_j \nonumber \\
   &= \sum_{j=1}^N \left( \omega_j^{\nabla^2} - \frac{\omega_g^{\nabla^2}}{\omega_g^n} \omega_j^n \right) u_j \nonumber \\
   &= \sum_{j=1}^N \tilde{\omega}_j^{\nabla^2} u_j
