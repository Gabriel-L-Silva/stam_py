...
def polygon_triangulation(polygon: Polygon, W, H, max_tri_area):
    points, indices = trimesh.creation.triangulate_polygon(polygon, 'pq30a'+str(max_tri_area if max_tri_area>0.0 else 1), engine='triangle')
    return trimesh.Trimesh(np.stack([points[:,0], points[:,1], np.zeros(len(points))], axis = 1), indices), polygon
